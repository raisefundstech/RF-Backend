    ...
    if (response)
      return res
        ...
        .json(
          new apiResponse(
            200,
            responseMessage.getDataSuccess("Your profile"),
            response[0]
          )
        );
    else
      return res
        .status(404)
        .json(
          new apiResponse(404, responseMessage.getDataNotFound("profile"), {})
        );
  } catch (error) {
    return res
      .status(500)
...
export const updateProfile = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (response) {
      ...
      return res
        .status(200)
        .json(new apiResponse(200, "Profile updated successfully", response));
    } else
      return res
        .status(404)
        .json(
          new apiResponse(404, "Database error while updating profile", {})
        );
  } catch (error) {
    return res
      .status(500)
...
export const switchWorkSpace = async (req: Request, res: Response) => {
  ...
  // console.log(req);
  try {
    let findWorkSpace = await workSpaceModel.findOne({
    ...
    if (findWorkSpace) {
      response = await userModel.findOneAndUpdate(
        ...
        { workSpaceId: ObjectId(req.params.id) },
        { new: true }
      );
      if (response)
        return res
          .status(200)
          .json(
            new apiResponse(200, "Work space switched successfully!", response)
          );
      else
        return res
          .status(400)
          .json(new apiResponse(400, "You can not switch workspace!", {}));
    } else {
      return res
        .status(400)
        .json(
...
export const getVolunteers = async (req: Request, res: Response) => {
  ...
  try {
    ...
    let getUserWorkSpace = await userModel.findOne(
    ...
    if (response)
      ...
      return res
        ...
        .json(
          new apiResponse(
            404,
            responseMessage.getDataNotFound("volunteers"),
            {}
          )
        );
  } catch (error) {
  ...
  try {
    ...
    if (userStatus?.userType === 1 || userStatus?.userType === 2) {
      if (!req.params.id) {
        return res
          .status(400)
          .json(new apiResponse(400, "Volunteer ID is required!", {}));
      }
      ...
      if (response) {
        return res
          ...
          .json(
            new apiResponse(
              200,
              responseMessage.getDataSuccess("volunteer"),
              response
            )
          );
      } else {
        return res
          ...
          .json(
            new apiResponse(
              404,
              responseMessage.getDataNotFound("volunteer"),
              {}
            )
          );
      }
...
export const updateVolunteerPosition = async (req: Request, res: Response) => {
  ...
  try {
    let previousUserState = await userModel.findOne({
    ...
    // Have to implement notification functionality after updating volunteer profile by the admin
      ...
      return res
        ...
        .json(
          new apiResponse(
            403,
            "You need to have admin privileges to update volunteer information",
            {}
          )
        );
  } catch (error) {
...
export const addVolunteer = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (response) {
      return res
        .status(200)
        .json(new apiResponse(200, "Volunteer added successfully!", {}));
    } else
      return res
        .status(404)
        .json(new apiResponse(404, "Volunteer add time some error occur!", {}));
  } catch (error) {
    return res
      .status(500)
      .json(new apiResponse(500, responseMessage?.internalServerError, error));
  ...
  try {
    ...
    if (response) {
      ...
      return res
        .status(404)
        .json(
          new apiResponse(501, responseMessage?.updateDataError("User"), {})
        );
    }
  } catch (error) {
...
export const logoutUser = async (req: Request, res: Response) => {
  ...
  let user: any = req.header("user");
  try {
    const user_session = await deleteSession(
      user._id,
    ...
    if (user_session.deletedCount > 0) {
      return res
        .status(200)
        .json(new apiResponse(200, responseMessage?.logoutSuccess, {}));
    } else {
      return res
        .status(404)
        .json(new apiResponse(501, responseMessage?.logoutFailure("User"), {}));
    }
  } catch (error) {
    return res
      .status(500)
    ...
    if (response) {
      return res
        ...
        .json(
          new apiResponse(
            200,
            responseMessage.getDataSuccess("unverified volunteers"),
            response
          )
        );
    } else {
      return res
        ...
        .json(
          new apiResponse(
            404,
            responseMessage.getDataNotFound("unverified volunteers"),
            {}
          )
        );
    }
