...
export const getMyEvents = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (response?.length > 0)
      return res
        ...
        .json(
          new apiResponse(
            200,
            responseMessage.getDataSuccess("events"),
            stadiumResponse
          )
        );
    else
    ...
    return res
      .status(500)
      .json(new apiResponse(500, responseMessage?.customMessage(error), error));
  }
};
...
export const getEvents = async (req: Request, res: Response) => {
  ...
  let userWorkSpace = await userModel.findOne(
    { _id: ObjectId(user._id) },
    { workSpaceId: 1, userType: 1 }
  );
  ...
  if (workSpaceId == null) {
    return res
      ...
      .json(
        new apiResponse(
          400,
          "Please provide a valid workspaceid and try again",
          {}
        )
      );
  }
  ...
  try {
    ...
    if (userInfo?.userType != 1) {
      return res
        .status(400)
        .json(
          new apiResponse(400, "You are not authorized to create event.", {})
        );
    }
    ...
    if (body.volunteerSize < 2)
      return res
        .status(400)
        .json(
          new apiResponse(400, "Please add volunteer size more than 1 . ", {})
        );
    ...
    if (validWorkSpace == null) {
      return res
        ...
        .json(
          new apiResponse(
            400,
            "Invalid workspace or stadium ID provided. Please provide valid IDs.",
            {}
          )
        );
    }
    ...
    if (response) {
      let userData = await userModel.find(
        ...
        { firstName: 1, lastName: 1, device_token: 1 }
      );
      ...
      return res
        ...
        .json(
          new apiResponse(
            200,
            responseMessage.addDataSuccess("event"),
            response
          )
        );
    } else
...
export const updateEvent = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (body.startTime || body.endTime) {
      if (
        new Date(body.startTime) < new Date() ||
        new Date(body.endTime) < new Date() ||
        new Date(body.startTime).toString() == new Date(body.endTime).toString()
      )
        return res
          .status(400)
          .json(new apiResponse(400, "Invalid start time or end time!", {}));
    }
    ...
    let userAuthority = await userModel.findOne(
      { _id: ObjectId(user?._id) },
      { userType: 1 }
    );
    ...
    if (response) {
      let updateRoomName = await roomModel.findOneAndUpdate(
        { eventId: ObjectId(response?._id), isActive: true },
        { roomName: response?.name + " " + "group" }
      );
      return res
        .status(200)
        .json(
          new apiResponse(200, responseMessage.updateDataSuccess("event"), {})
        );
    } else
      return res
        .status(400)
        .json(
          new apiResponse(400, responseMessage.updateDataError("event"), {})
        );
  } catch (error) {
    return res
      .status(500)
...
export const getEventById = async (req: Request, res: Response) => {
  ...
  try {
    ...
    response.volunteerRequest =
      volunteerRequestInfo?.volunteerRequest === undefined
        ? []
        : volunteerRequestInfo?.volunteerRequest;
    response = {
      ...response._doc,
    ...
    if (response)
      return res
        ...
        .json(
          new apiResponse(
            200,
            responseMessage.getDataSuccess("event"),
            response
          )
        );
    else
      return res
        .status(400)
        .json(
          new apiResponse(400, responseMessage.getDataNotFound("event"), {})
        );
  } catch (error) {
    return res
      .status(500)
  ...
  try {
    ...
    if (response)
      return res
        .status(200)
        .json(
          new apiResponse(200, responseMessage.deleteDataSuccess("event"), {})
        );
    else
      return res
        .status(400)
...
export const get_event_pagination = async (req: Request, res: Response) => {
  reqInfo(req);
  let { workSpaceId, search, page, limit } = req.body,
    event_data: any,
    match: any = {},
    event_count: any;
  try {
                        ...
                        $arrayElemAt: [
                          {
                            $filter: {
                              input: "$volunteerData",
                              cond: {
                                $eq: ["$$this._id", "$$request.volunteerId"],
                              },
                            },
                          },
                          0,
    ...
    return res.status(200).json(
      new apiResponse(200, responseMessage.getDataSuccess("event"), {
...
export const apply = async (req: Request, res: Response) => {
  ...
  try {
    let getUser = await userModel.findOne(
      { _id: ObjectId(user._id) },
      { workSpaceId: 1, userStatus: 1, userType: 1 }
    );

    if (getUser?.workSpaceId != body.workSpaceId) {
      throw new Error(
        "You can't apply to an event located in different workstation, please switch the workstation and try again."
      );
    }

    if (getUser?.userStatus != 1) {
      throw new Error(
        "You are not authorized to apply for an event, please contact your admin."
      );
    }
    ...
    if (result?.volunteerRequest.length % 10 === 0) {
      ...
      var getEvent = await eventModel.findOne({
        _id: ObjectId(body._id),
        isActive: true,
      });
      const eventDetails = await getStadiumDetailsByWorkSpace(
        getEvent?.workSpaceId,
        getEvent?.stadiumId
      );
    ...
    return res
      .status(200)
      .json(
        new apiResponse(200, "You have succeessfully applied for the event", {})
      );
  } catch (error) {
    return res
      .status(500)
      .json(
        new apiResponse(500, responseMessage?.customMessage(error), { error })
      );
  }
};
...
export const withdraw = async (req: Request, res: Response) => {
  ...
  try {
    ...
    let userWorkSpace = await getUser(user?._id, true).then(
    ...
    return res
      ...
      .json(
        new apiResponse(
          200,
          "You have successfully withdrawn from the event",
          {}
        )
      );
  } catch (error) {
...
export const updateVolunteers = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (userAuthority?.userType != 1 && userAuthority?.userType != 2) {
      throw new Error(
        "You are not authorized to update volunteer request status."
      );
    }
    ...
    if (getEvent == null) {
      return res
        ...
        .json(
          new apiResponse(
            400,
            "Invalid event id. Please provide valid event id.",
            {}
          )
        );
    }

    let responseData = getEvent?.volunteerRequest?.filter(
      (data) => data?.requestStatus === "APPROVED"
    );
    let approvedVolunteersInBody = body?.volunteerRequest?.filter(
      (data) => data?.requestStatus === "APPROVED"
    );
    if (
      getEvent?.volunteerSize <
      responseData?.length + approvedVolunteersInBody?.length
    ) {
      return res
        .status(400)
        .json(
          new apiResponse(
            400,
            `The current request exceeds the limit of available volunteer slots. You can only approve volunteers within the limit of ${getEvent?.volunteerSize}.`,
            {}
          )
        );
    }
    ...
    const notificationResponse = body?.volunteerRequest.map(
      async (data: any) => {
        const result = await updateVolunteersRequestStatus(
          req,
          data?.volunteerId,
          data?.requestStatus,
          data?.userNote
        ...
        if (tokens?.length > 0) {
          ...
          console.log(
            "User has not allowed the notifications or unable to send notifications."
          );
        }
      }
    );
    ...
    return res
      ...
      .json(
        new apiResponse(
          200,
          "Volunteers event status has been updated successfully",
          {}
        )
      );
  } catch (error) {
 ...
 */
export const get_event_pagination_for_volunteers = async (
  req: Request,
  res: Response
) => {
  reqInfo(req);
...
export const getVolunteerByEvent = async (req: Request, res: Response) => {
  ...
  try {
    ...
    let responseData = response[0]?.volunteerRequest.filter(
    ...
    if (response)
      return res
        ...
        .json(
          new apiResponse(
            200,
            "You have successfully added volunteers to event!",
            {}
          )
        );
  } catch (error) {
...
export const volunteerCheckIn = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (requestStatus == null || requestStatus.length === 0) {
      return res
        ...
        .json(
          new apiResponse(
            400,
            "Invalid request. Please provide VolunteerRequest data.",
            {}
          )
        );
    }
    ...
    const volunteersCheckInStatus = body?.volunteerRequest.map(
      async (data: any) => {
        response = await updateVolunteersCheckInStatus(
          body?._id,
          data?.volunteerId
        );
        if (response?.error) {
    ...
    return res
      ...
      .json(
        new apiResponse(
          200,
          "Volunteers checkedIn status has been updated successfully",
          {}
        )
      );
  } catch (error) {
...
export const volunteerCheckOut = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const stadiumInfo = await getStadiumDetailsByWorkSpace(
      eventInfo?.workSpaceId,
      eventInfo?.stadiumId
    );

    // Update check-in status for each volunteer
    const volunteersCheckOutStatus = body?.volunteerRequest.map(
      async (data: any) => {
        // update user attendance in attendance table
        response = await updateVolunteersCheckOutStatus(
          body?._id,
          data?.volunteerId
        );
        ...
        if (userInfo?.device_token?.length > 0) {
          let userTokenMapper = mapTokensToUser(
            data?.volunteerId,
    ...
    return res
      ...
      .json(
        new apiResponse(
          200,
          "Volunteers attendance has been updated successfully",
          {}
        )
      );
  } catch (error) {
