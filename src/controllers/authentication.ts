...
export const userSignUp = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (isAlready?.email == body?.email)
      return res
        .status(409)
        .json(new apiResponse(409, responseMessage?.alreadyEmail, {}));
    if (isAlready?.mobileNumber == body?.mobileNumber)
      return res
        .status(409)
        .json(new apiResponse(409, responseMessage?.alreadyMobileNumber, {}));

    body.volunteerId = await generateVolunteerCode();

    let checkUserExists: any = await userModel.findOne({
    ...
    if (checkUserExists) {
      ...
      if (response) {
        return res
          .status(200)
          .json(new apiResponse(200, responseMessage?.signupSuccess, {}));
      } else {
        return res
          .status(501)
          .json(new apiResponse(501, "Something went wrong", { response }));
      }
    }
...
export const userSignIn = async (req: Request, res: Response) => {
  ...
  try {
    ...
    let findData: any = await userModel.findOneAndUpdate(
      {
        $and: [
          { $or: [{ userType: 0 }, { userType: 1 }, { userType: 2 }] },
          { mobileNumber: body.mobileNumber, isActive: true },
        ],
      },
    ...
    if (!findData) {
      return res
        .status(400)
        .json(new apiResponse(400, responseMessage?.invalidMobileNumber, {}));
    }

    const messageBody = `${findData?.otp} is your OTP to log in to Raise funds. Code will expire in 5 minutes.`;

    if (findData) {
      var sms_response = await sendLoginSMS(
        findData?.mobileNumber,
        messageBody
      );
      logger.info(sms_response.MessageResponse);
      var status =
        sms_response?.MessageResponse?.Result[findData?.mobileNumber]
          ?.DeliveryStatus;
      if (sms_response.$metadata?.httpStatusCode == 200) {
        return res
          ...
          .json(
            new apiResponse(
              200,
              `OTP has been sent to this ${findData.mobileNumber}`,
              { status }
            )
          );
      }
    ...
    let error_message =
      "SMS rejected for delivery to: " + findData?.mobileNumber;
    return res
      .status(501)
      .json(new apiResponse(501, "Something went wrong", { error_message }));
  } catch (error) {
    logger.error(error);
    return res
      .status(500)
...
export const otpVerification = async (req: Request, res: Response) => {
  ...
  try {
    ...
    let findUser = await userModel.findOne(
      ...
      { _id: 1, otp: 1, otpExpireTime: 1 }
    );

    if (!findUser)
      return res
        .status(400)
        .json(new apiResponse(400, responseMessage?.invalidOTP, {}));
    ...
    if (findUser.otpExpireTime < new Date()) {
      return res
        .status(410)
        .json(new apiResponse(410, responseMessage?.expireOTP, {}));
    }

    if (findUser) {
      // Only one device can be logged in at a time
      let LoggedIn = await userSessionModel.findOne({
      ...
      if (LoggedIn) {
        ...
        if (delete_sessions?.error) {
          throw new Error(
            responseMessage?.customMessage("Error in deleting sessions")
          );
        }
        logout_response =
          delete_sessions != null
            ? delete_sessions
            : "User have no active session";
      ...
      return res
        .status(200)
        .json(
          new apiResponse(200, verfication_logout_endpoint_response, responseIs)
        );
    } else {
      return res
        .status(501)
        .json(new apiResponse(501, "Something went wrong", {}));
    }
  } catch (error) {
    console.log(error);
    return res
...
export const sendOTP = async (req: Request, res: Response) => {
  try {
    ...
    var sms_response = {
      ...
      deliveryStatusCode:
        response?.MessageResponse?.Result[mobileNumber]?.StatusCode,
      deliveryStatus:
        response?.MessageResponse?.Result[mobileNumber]?.DeliveryStatus,
    ...
    if (response) {
      return res
        ...
        .json(
          new apiResponse(
            200,
            `OTP has been successfully sent to mobile number ${body.mobileNumber}`,
            { sms_response }
          )
        );
    } else {
      return res
        .status(400)
        .json(new apiResponse(400, "Failed to send OTP", { response }));
    }
  } catch (error) {
...
export const verifyOTP = async (req: Request, res: Response) => {
  try {
    ...
    if (response["VerificationResponse"]["Valid"] == true) {
      return res
        .status(200)
        .json(new apiResponse(200, `OTP has been successfully verified`, {}));
    } else {
      return res
        .status(400)
        .json(
          new apiResponse(
            400,
            "Please submit a valid OTP or make a request to get the latest OTP",
            {}
          )
        );
    }
    ...
    if (response) {
      return res
        ...
        .json(
          new apiResponse(
            200,
            `OTP has been sent to this mobile number ${body.mobileNumber}`,
            { email_message }
          )
        );
    } else {
      ...
      return res
        .status(501)
        .json(
          new apiResponse(501, "Error in send sms please try again!", {
            email_message,
          })
        );
    }
...
export const userLogout = async (req: Request, res: Response) => {
  ...
  try {
    await userModel.findOneAndUpdate(
      ...
      { $pull: { device_token: req.body?.device_token } }
    );
    return res
      .status(200)
      .json(new apiResponse(200, responseMessage?.logout, {}));
  } catch (error) {
    console.log(error);
    return res
      .status(500)
...
export const validate = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (response.length === 0) {
      return res
        .status(200)
        .json(new apiResponse(200, "Mobile Number does not exist", {}));
    } else {
      return res
        .status(409)
